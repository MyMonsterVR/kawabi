...
fun AnimeBrowseScreen(
    ...
    viewModel: AnimeBrowseViewModel = koinViewModel(),
) {
        ...
        ResponsiveContainer(modifier = Modifier.padding(padding)) {
            Column(modifier = Modifier.fillMaxSize().background(NightSession.Background)) {
                Row(
                    ...
                    NightChip(
                        label = AnimeBrowseSort.POPULAR.label,
                        selected = sort == AnimeBrowseSort.POPULAR,
                        onClick = { viewModel.setSort(AnimeBrowseSort.POPULAR) },
                    )
                    if (supportsLatest) {
                        NightChip(
                            label = AnimeBrowseSort.LATEST.label,
                            selected = sort == AnimeBrowseSort.LATEST,
                            onClick = { viewModel.setSort(AnimeBrowseSort.LATEST) },
                        )
                    }
                }
